...
ll divide(ll a, ll b) {
 ...
 vector<int>minsubDiff(vector<int>&v,int n,int &range){
     f(i, range + 1) dp[0][i] = false;
     f(i, n + 1) dp[i][0] = true;
...
int32_t main() {
    ...
    while (t--) {
        ...
        f(i, n) {cin >> v[i];
        range+=v[i];}
        dp=vector<vector<int>>(n+1,vector<int>(range+1,false));
        vector<int>res= minsubDiff(v,v.size(),range);

        int mn=INT_MAX;
        
        for(auto num:res){
            mn=min(mn,range-2*num);
        }
        cout<<mn<<endl;
